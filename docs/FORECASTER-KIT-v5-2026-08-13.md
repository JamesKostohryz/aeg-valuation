...
Valid against commit `db1028f` on `github.com/JamesKostohryz/aeg-valuation`. Regression harness green; four-method tie `8.396062e-16`, and as of v3 the published value is **wholly inside that tie** — there is no longer any component sitting outside it. v5 moves no published number: PepsiCo's `$110.980452128219` tied / `$116.2259` headline per anchor share is unchanged, and PepsiCo is the only company that publishes one.

# FORECASTER KIT — CURRENT VERSION (v5, 2026-08-13)
...
Valid against commit `db1028f` on `github.com/JamesKostohryz/aeg-valuation`. Regression harness green; four-method tie `8.396062e-16`, and as of v3 the published value is **wholly inside that tie** — there is no longer any component sitting outside it. v5 moves no published number **by its own action**: `companies/PEP.yaml` and `companies/PEP.forecast.json` are untouched, and on the first run after this change PepsiCo cleared every gate with the four-method tie at `4.91e-10` and `cfg_N_reviewed: True`. PepsiCo is the only company that publishes a number at all.

> **A property of this system that is easy to mis-state, and was mis-stated in the first draft of this preamble.** PepsiCo's headline is *not* stable across runs. The pipeline repoints rates and price live on every run, so the same reviewed forecast produced `$116.2259` against a real price of `$144.3782` at commit `fb4927c` and `$115.7595` against `$146.3768` at `ecf6a58` — a move of 0.40% caused by the long-run real cost of equity going from `5.4877%` to `5.5279%`, four basis points. Nothing about the forecast changed. **Always quote a valuation together with the commit and the run that produced it**, and never carry a headline number forward from a document without re-reading `outputs/<TICKER>_summary.csv`. Four basis points on the long end is worth roughly half a dollar a share here, which is also a useful reminder of how much work that single rate does — standing failure mode two, visible in the ordinary operation of the system.

**What changed in v5.** Three things, all from the Coca-Cola onboarding on 2026-08-13, none of them a new gate and none of them touching the tie.

1. **A new section 7: the AEG value test.** The kit had never stated what a forecast has to beat for abnormal earnings growth to be positive at all. It now quotes Valuation rows 22 and 23 verbatim and gives the hurdle in both useful forms — return on retained earnings against the real cost of equity, and real earnings growth against the real cost of equity times the prior year's retention rate. Section 7.5 adds the related warning that the engine does not value the "comparable" earnings series a company guides on.
2. **A corrected instruction and a new pin.** `pipeline/aeg_schedule.py` told forecasters to compare the published nominal `rore` column against the published nominal `coe` column. That test disagreed with the engine in eight of Coca-Cola's twelve forecast years. The docstring is corrected and the identity is now pinned by `pipeline/test_aeg_schedule.py` check 6. No published number moved; this was a wrong instruction attached to correct numbers.
3. **Thirteen configs no longer assert a review that did not happen.** Every company except PepsiCo carried `forecast.reviewed: true` two lines under its own comment saying the horizon was never studied. All thirteen are now `reviewed: false`, and `pipeline/test_config.py`'s fleet-wide check has been **strengthened**, not relaxed: `forecast.reviewed` and the existence of a `companies/<T>.forecast.json` must now agree, in both directions. See section 9 for what this does to the gated set.
...
**PepsiCo — done, published, and reproducible from the repository.** The whole four-round protocol
was completed on 2026-08-12 and the reviewed forecast became a repository artifact on 2026-08-13
(`companies/PEP.forecast.json`), so the pipeline reproduces the published valuation without any
Cockpit involvement. Gate A passes at N=12 (abnormal earnings growth −$0.0082/sh, factor 0.378,
discarded tail 0.64% of value), Gate B passes (earnings gap 0.4% of EPS), the funding gate passes in
all twelve years, the terminal payout gate passes at 78.0% of normalized EPS $9.2879, and the
four-method tie holds at 3.47e-15. **Tied equity $110.980452128219 per anchor share; headline
$116.2259; bull $124.4046; bear $75.7993; expected value $103.1110 against a real price of
$144.3782.**
...
**Coca-Cola — Round 1 done, Round 2 next.** Round 1 landed 2026-08-13 as a qualitative brief and
deliberately does not propose a horizon. Three things from it a forecaster should know before
picking up any second company:

- It is refused by the **funding gate** rather than the truncation gates, and the fix is a real
  distribution policy: Coca-Cola's own investor disclosure states it repurchases only enough stock
  to offset employee option dilution, net repurchases were $0.4bn in 2025 against $8.8bn of
  dividends, and the share count is *higher* than it was in 2018. The default overlay's three
  percent is roughly thirty times the company's stated policy.
